...
class TemperatureHumiditySensorController(
    ...
    private val log: Logger = LoggerFactory.getLogger(TemperatureHumiditySensorController::class.java.name)
    ...
    fun createTemperatureHumiditySensor(@SwaggerRequestBody(description = "Request to add a new temperature and humidity sensor") @RequestBody request: CreateTemperatureHumiditySensorRequest): ResponseEntity<String> {
        ...
        val sensor = temperatureHumiditySensorService.createSensor(request) ?: return ResponseEntity.badRequest()
            .body("Failed to create temperature and humidity sensor")
    ...
    @ApiResponses(
        value = [
            ApiResponse(
                responseCode = "200",
                description = "Successfully retrieved the temperature and humidity sensor"
            ),
            ...
            ApiResponse(
                responseCode = "404",
                description = "The temperature and humidity sensor you were trying to reach is not found"
            )
        ]
    )
    ...
    @ApiResponses(
        value = [
            ApiResponse(
                responseCode = "200",
                description = "Successfully retrieved all temperature and humidity sensors"
            ),
            ApiResponse(responseCode = "401", description = "You are not authorized to view the resource"),
            ApiResponse(
                responseCode = "403",
    ...
    fun updateTemperatureHumiditySensor(
        ...
        val sensor =
            temperatureHumiditySensorService.updateSensor(sensorId, request) ?: return ResponseEntity.badRequest()
                .body("Failed to update temperature and humidity sensor")
        replConfig.regenerateSingleModel().invoke("temperatureHumiditySensors")
    ...
    fun deleteTemperatureHumiditySensor(@PathVariable sensorId: String): ResponseEntity<String> {
        ...
        if (!deleted) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body("Temperature and humidity sensor $sensorId not found")
        }
