...
class MoistureSensorController (
    ...
    fun createMoistureSensor(@SwaggerRequestBody(description = "Request to add a new moisture sensor") @RequestBody request: CreateMoistureSensorRequest): ResponseEntity<String> {
        ...
        val sensor = moistureSensorService.createSensor(request) ?: return ResponseEntity.badRequest().body("Failed to create moisture sensor")
    ...
    fun updateMoistureSensor(@PathVariable sensorId: String, @SwaggerRequestBody(description = "Request to update an existing moisture sensor") @RequestBody request: UpdateMoistureSensorRequest): ResponseEntity<String> {
        log.info("Updating moisture sensor $sensorId with request $request")

        val sensor = moistureSensorService.updateSensor(sensorId, request) ?: return ResponseEntity.badRequest().body("Failed to update moisture sensor")
        replConfig.regenerateSingleModel().invoke("moistureSensors")
